...
module.exports = (function(Shover) {
  ...
  Shover = function(options) {
    ...
    this.id = uuid()
    this.baseurl = util.format("http://%s:%d", options.http.host, options.http.port)
    this.connections = {}
    this.channels = {}
  ...
  Shover.prototype.racket = function(callback) {
    this.redis.eval("return redis.call('mget', unpack(redis.call('keys', 'server:*:url')))", 0, callback)
  }
  ...
  function bind(server, shover) {
    shover.redis.set(shover.rid("url"), shover.baseurl)
